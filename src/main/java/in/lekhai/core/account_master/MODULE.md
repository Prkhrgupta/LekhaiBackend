...
- **Purchase & sale ledger settings** — for each purchase/sale ledger, which ledgers its tax components (CGST/SGST/IGST) post to. This is what lets a purchase or sales voucher automatically split the bill into the correct duty & tax accounts. (Cess rates live on the commodity; cess ledgers live in the general setting below.)
- **General ledger setting** — one row per shop holding the shared defaults: freight/packing and round-off ledgers, TDS (percentage + ledger), TCS (percentage + ledger), and the output/input cess ledgers.
...
- `GeneralLedgerSetting*` — singleton-per-shop shared defaults (freight/packing, round-off, TDS/TCS, output/input cess ledgers); singleton GET + PUT upsert, no dropdown/summary.
